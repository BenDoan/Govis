...
func (t *Tracker) Start(config *jconfig.Config) {
	...
	t.minTimeDiff = config.GetInt("MinTimeDiff")
...
func (t *Tracker) PrintStatus() {
	...
	if !(timeDiff < t.minTimeDiff) && t.IsValidWindow(t.lastWindow) {
...
func (t *Tracker) IsValidWindow(lastWindow string) (isValid bool) {
	...
	for _, val := range t.ignorePatterns {
		lowerLastWindow := strings.ToLower(lastWindow)
		if str, ok := val.(string); ok && strings.Contains(lowerLastWindow, strings.ToLower(str)) {
			isValid = false
			break
		}
	}
